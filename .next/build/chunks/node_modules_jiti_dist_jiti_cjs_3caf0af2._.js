...
"[project]/node_modules/jiti/dist/jiti.cjs [postcss] (ecmascript)": ((__turbopack_context__) => {

var { m: module, e: exports } = __turbopack_context__;
{
(()=>{
    var __webpack_modules__ = {
        "./node_modules/.pnpm/mlly@1.7.4/node_modules/mlly/dist lazy recursive": (module1)=>{
            ...
            webpackEmptyAsyncContext.keys = ()=>[], webpackEmptyAsyncContext.resolve = webpackEmptyAsyncContext, webpackEmptyAsyncContext.id = "./node_modules/.pnpm/mlly@1.7.4/node_modules/mlly/dist lazy recursive", module1.exports = webpackEmptyAsyncContext;
        }
    ...
    (()=>{
        ...
        const external_node_os_namespaceObject = __turbopack_context__.r("[externals]/node:os [external] (node:os, cjs)");
        ...
        Parser.prototype.parse = function() {
            ...
            for(var i = this.scopeStack.length - 1; i >= 0; i--){
                var flags = this.scopeStack[i].flags;
                if (768 & flags || 2 & flags && !(16 & flags)) return !0;
            }
            return !1;
        ...
        pp$8.isLet = function(context) {
            ...
            if (isAwaitUsing) {
                var after, awaitEndPos = next + 5;
                if ("using" !== this.input.slice(next, awaitEndPos) || awaitEndPos === this.input.length || isIdentifierChar(after = this.input.charCodeAt(awaitEndPos)) || after > 55295 && after < 56320) return !1;
            ...
            if (isFor) {
                var after$1, ofEndPos = next + 2;
                if (!("of" !== this.input.slice(next, ofEndPos) || ofEndPos !== this.input.length && (isIdentifierChar(after$1 = this.input.charCodeAt(ofEndPos)) || after$1 > 55295 && after$1 < 56320))) return !1;
            }
            var ch = this.input.charCodeAt(next);
            return isIdentifierStart(ch, !0) || 92 === ch;
            ...
            switch(this.isLet(context) && (starttype = types$1._var, kind = "let"), starttype){
                ...
                default:
                    ...
                    if (usingKind) return topLevel && "script" === this.options.sourceType && this.raise(this.start, "Using declaration cannot appear in the top level when source type is `script`"), "await using" === usingKind && (this.canAwait || this.raise(this.start, "Await using cannot appear outside of async function"), this.next()), this.next(), this.parseVar(node, !1, usingKind), this.semicolon(), this.finishNode(node, "VariableDeclaration");
            ...
            if (usingKind) {
                ...
                return this.next(), "await using" === usingKind && this.next(), this.parseVar(init$2, !0, usingKind), this.finishNode(init$2, "VariableDeclaration"), this.parseForAfterInit(node, init$2, awaitAt);
            }
            ...
            return (this.type === types$1._in || this.options.ecmaVersion >= 6 && this.isContextual("of")) && 1 === init.declarations.length ? (this.options.ecmaVersion >= 9 && (this.type === types$1._in ? awaitAt > -1 && this.unexpected(awaitAt) : node.await = awaitAt > -1), this.parseForIn(node, init)) : (awaitAt > -1 && this.unexpected(awaitAt), this.parseFor(node, init));
        ...
        pp$8.parseCatchClauseParam = function() {
            ...
            for(node.declarations = [], node.kind = kind;;){
                ...
                if (this.parseVarId(decl, kind), this.eat(types$1.eq) ? decl.init = this.parseMaybeAssign(isFor) : allowMissingInitializer || "const" !== kind || this.type === types$1._in || this.options.ecmaVersion >= 6 && this.isContextual("of") ? allowMissingInitializer || "using" !== kind && "await using" !== kind || !(this.options.ecmaVersion >= 17) || this.type === types$1._in || this.isContextual("of") ? allowMissingInitializer || "Identifier" === decl.id.type || isFor && (this.type === types$1._in || this.isContextual("of")) ? decl.init = null : this.raise(this.lastTokEnd, "Complex binding patterns require an initialization value") : this.raise(this.lastTokEnd, "Missing initializer in " + kind + " declaration") : this.unexpected(), node.declarations.push(this.finishNode(decl, "VariableDeclarator")), !this.eat(types$1.comma)) break;
            }
            return node;
        }, pp$8.parseVarId = function(decl, kind) {
            decl.id = "using" === kind || "await using" === kind ? this.parseIdent() : this.parseBindingAtom(), this.checkLValPattern(decl.id, "var" === kind ? 1 : 2, !1);
        };
        ...
        pp$8.parseFunction = function(node, statement, allowExpressionBody, isAsync, forInit) {
            ...
            return checkKeyName(field, "constructor") ? this.raise(field.key.start, "Classes can't have a field named 'constructor'") : field.static && checkKeyName(field, "prototype") && this.raise(field.key.start, "Classes can't have a static field named 'prototype'"), this.eat(types$1.eq) ? (this.enterScope(576), field.value = this.parseMaybeAssign(), this.exitScope()) : field.value = null, this.semicolon(), this.finishNode(field, "PropertyDefinition");
        ...
        pp$5.checkPropClash = function(prop, propHash, refDestructuringErrors) {
            ...
            var node = this.startNode();
            return node.value = value, node.raw = this.input.slice(this.start, this.end), 110 === node.raw.charCodeAt(node.raw.length - 1) && (node.bigint = null != node.value ? node.value.toString() : node.raw.slice(0, -1).replace(/_/g, "")), this.next(), this.finishNode(node, "Literal");
        ...
        pp$5.parseNew = function() {
            ...
            (this.inGenerator && "yield" === name && this.raiseRecoverable(start, "Cannot use 'yield' as identifier inside a generator"), this.inAsync && "await" === name && this.raiseRecoverable(start, "Cannot use 'await' as identifier inside an async function"), 259 & this.currentThisScope().flags || "arguments" !== name || this.raiseRecoverable(start, "Cannot use 'arguments' in class field initializer"), !this.inClassStaticBlock || "arguments" !== name && "await" !== name || this.raise(start, "Cannot use " + name + " in class static initialization block"), this.keywords.test(name) && this.raise(start, "Unexpected keyword '" + name + "'"), this.options.ecmaVersion < 6 && -1 !== this.input.slice(start, end).indexOf("\\")) || (this.strict ? this.reservedWordsStrict : this.reservedWords).test(name) && (this.inAsync || "await" !== name || this.raiseRecoverable(start, "Cannot use keyword 'await' outside an async function"), this.raiseRecoverable(start, "The keyword '" + name + "' is reserved"));
        ...
        pp$3.enterScope = function(flags) {
            ...
            for(var i = this.scopeStack.length - 1;; i--){
                ...
                if (771 & scope.flags) return scope;
            }
        ...
        const external_node_module_namespaceObject = __turbopack_context__.r("[externals]/node:module [external] (node:module, cjs)"), external_node_fs_namespaceObject = __turbopack_context__.r("[externals]/node:fs [external] (node:fs, cjs)");
        ...
        function withTrailingSlash(input = "", respectQueryAndFragment) {
            ...
            if (-1 !== fragmentIndex && (path = input.slice(0, fragmentIndex), fragment = input.slice(fragmentIndex), !path)) return fragment;
        ...
        function pathe_M_eThtNZ_normalizeWindowsPath(input = "") {
            return input ? input.replace(/\\/g, "/").replace(_DRIVE_LETTER_START_RE, (r)=>r.toUpperCase()) : input;
        }
        const _UNC_REGEX = /^[/\\]{2}/, _IS_ABSOLUTE_RE = /^[/\\](?![/\\])|^[/\\]{2}(?!\.)|^[A-Za-z]:[/\\]/, _DRIVE_LETTER_RE = /^[A-Za-z]:$/, _EXTNAME_RE = /.(\.[^./]+|\.)$/, pathe_M_eThtNZ_normalize = function(path) {
            if (0 === path.length) return ".";
            const isUNCPath = (path = pathe_M_eThtNZ_normalizeWindowsPath(path)).match(_UNC_REGEX), isPathAbsolute = isAbsolute(path), trailingSeparator = "/" === path[path.length - 1];
        ...
        }, pathe_M_eThtNZ_join = function(...segments) {
            let path = "";
            for (const seg of segments)if (seg) if (path.length > 0) {
                const pathTrailing = "/" === path[path.length - 1], segLeading = "/" === seg[0];
                path += pathTrailing && segLeading ? seg.slice(1) : pathTrailing || segLeading ? seg : `/${seg}`;
            } else path += seg;
        ...
        const resolve = function(...arguments_) {
            ...
            for(let index = (arguments_ = arguments_.map((argument)=>pathe_M_eThtNZ_normalizeWindowsPath(argument))).length - 1; index >= -1 && !resolvedAbsolute; index--){
        ...
        const isAbsolute = function(p) {
            ...
            if (".." === p) return "";
            const match = _EXTNAME_RE.exec(pathe_M_eThtNZ_normalizeWindowsPath(p));
            return match && match[1] || "";
        }, pathe_M_eThtNZ_dirname = function(p) {
            const segments = pathe_M_eThtNZ_normalizeWindowsPath(p).replace(/\/$/, "").split("/").slice(0, -1);
            ...
            const segments = pathe_M_eThtNZ_normalizeWindowsPath(p).split("/");
            let lastSegment = "";
        ...
        }, external_node_url_namespaceObject = __turbopack_context__.r("[externals]/node:url [external] (node:url, cjs)"), external_node_assert_namespaceObject = __turbopack_context__.r("[externals]/node:assert [external] (node:assert, cjs)"), external_node_process_namespaceObject = __turbopack_context__.r("[externals]/node:process [external] (node:process, cjs)"), external_node_path_namespaceObject = __turbopack_context__.r("[externals]/node:path [external] (node:path, cjs)"), external_node_v8_namespaceObject = __turbopack_context__.r("[externals]/node:v8 [external] (node:v8, cjs)"), external_node_util_namespaceObject = __turbopack_context__.r("[externals]/node:util [external] (node:util, cjs)"), BUILTIN_MODULES = new Set(external_node_module_namespaceObject.builtinModules);
        ...
        function moduleResolve(specifier, base, conditions, preserveSymlinks) {
            ...
            return external_node_assert_namespaceObject(void 0 !== resolved, "expected to be defined"), "file:" !== resolved.protocol ? resolved : function(resolved, base) {
        ...
        const pathSeparators = new Set([
            ...
            "\\",
            void 0
        ]), normalizedAliasSymbol = Symbol.for("pathe:normalizedAlias"), utils_SLASH_RE = /[/\\]/;
        ...
        const package_namespaceObject = {
            rE: "2.5.1"
        }, external_node_crypto_namespaceObject = __turbopack_context__.r("[externals]/node:crypto [external] (node:crypto, cjs)");
        var external_node_crypto_default = __webpack_require__.n(external_node_crypto_namespaceObject);
        const dist_r = Object.create(null), dist_i = (e)=>globalThis.process?.env || globalThis.Deno?.env.toObject() || globalThis.__env__ || (e ? dist_r : globalThis), dist_o = new Proxy(dist_r, {
            get: (e, s)=>dist_i()[s] ?? dist_r[s],
            has: (e, s)=>s in dist_i() || s in dist_r,
            set: (e, s, E)=>(dist_i(!0)[s] = E, !0),
            deleteProperty (e, s) {
                if (!s) return !1;
                return delete dist_i(!0)[s], !0;
            },
            ownKeys () {
                const e = dist_i(!0);
        ...
        const I = globalThis.process?.platform || "", T = dist_n(dist_o.CI) || !1 !== l.ci, R = dist_n(globalThis.process?.stdout && globalThis.process?.stdout.isTTY), a = (dist_n(dist_o.DEBUG), "test" === dist_t || dist_n(dist_o.TEST)), A = (dist_n(dist_o.MINIMAL), /^win/i.test(I)), C = (/^linux/i.test(I), /^darwin/i.test(I), !dist_n(dist_o.NO_COLOR) && (dist_n(dist_o.FORCE_COLOR) || (R || A) && dist_o.TERM), (globalThis.process?.versions?.node || "").replace(/^v/, "") || null), W = (Number(C?.split(".")[0]), globalThis.process || Object.create(null)), _ = {
            versions: {}
        }, O = (new Proxy(W, {
            get: (e, s)=>"env" === s ? dist_o : s in e ? e[s] : s in _ ? _[s] : void 0
        }), "node" === globalThis.process?.release?.name), dist_c = !!globalThis.Bun || !!globalThis.process?.versions?.bun, D = !!globalThis.Deno, L = !!globalThis.fastly, F = [
            [
                !!globalThis.Netlify,
                "netlify"
            ],
        ...
        const external_node_tty_namespaceObject = __turbopack_context__.r("[externals]/node:tty [external] (node:tty, cjs)"), hasColors = external_node_tty_namespaceObject?.WriteStream?.prototype?.hasColors?.() ?? !1, base_format = (open, close)=>{
        ...
        function hash(content, len = 8) {
            return ((function() {
                if (void 0 !== _fipsMode) return _fipsMode;
                try {
                    return _fipsMode = !!external_node_crypto_default().getFips?.(), _fipsMode;
                } catch  {
                    return _fipsMode = !1, _fipsMode;
                }
            })() ? external_node_crypto_default().createHash("sha256") : external_node_crypto_default().createHash("md5")).update(content).digest("hex").slice(0, len);
        }
        const debugMap = {
            true: green("true"),
            false: yellow("false"),
        ...
        function jitiInteropDefault(ctx, mod) {
            return ctx.opts.interopDefault ? function(mod) {
                ...
                const def = mod.default, defType = typeof def, defIsNil = null == def, defIsObj = "object" === defType || "function" === defType;
                if (defIsNil && mod instanceof Promise) return mod;
                return new Proxy(mod, {
                    get (target, prop, receiver) {
                        if ("__esModule" === prop) return !0;
                        if ("default" === prop) return defIsNil ? mod : def;
        ...
        function jitiResolve(ctx, id, options) {
            ...
            ctx.alias && (id = function(path, aliases) {
                const _path = pathe_M_eThtNZ_normalizeWindowsPath(path);
                aliases = normalizeAliases(aliases);
                for (const [alias, to] of Object.entries(aliases)){
                    if (!_path.startsWith(alias)) continue;
                    const _alias = utils_hasTrailingSlash(alias) ? alias.slice(0, -1) : alias;
                    if (utils_hasTrailingSlash(_path[_alias.length])) return pathe_M_eThtNZ_join(to, _path.slice(alias.length));
                }
                return _path;
            }(id, ctx.alias));
            let parentURL = options?.parentURL || ctx.url;
            isDir(parentURL) && (parentURL = pathe_M_eThtNZ_join(parentURL, "_index.js"));
        ...
        function tryNativeRequireResolve(ctx, id, parentURL, options) {
            try {
                return ctx.nativeRequire.resolve(id, {
                    ...options,
                    paths: [
                        pathe_M_eThtNZ_dirname(fileURLToPath(parentURL)),
                        ...options?.paths || []
                    ]
                });
            } catch  {}
        }
        const external_node_perf_hooks_namespaceObject = __turbopack_context__.r("[externals]/node:perf_hooks [external] (node:perf_hooks, cjs)"), external_node_vm_namespaceObject = __turbopack_context__.r("[externals]/node:vm [external] (node:vm, cjs)");
        ...
        function nativeImportOrRequire(ctx, id, async) {
            return async && ctx.nativeImport ? ctx.nativeImport(function(id) {
                return A && isAbsolute(id) ? pathToFileURL(id) : id;
        ...
        function getCache(ctx, topts, get) {
            ...
            let cacheName = `${basename(pathe_M_eThtNZ_dirname(topts.filename))}-${function(path) {
                const base = path.split(utils_SLASH_RE).pop();
                if (!base) return;
                const separatorIndex = base.lastIndexOf(".");
                return separatorIndex <= 0 ? base : base.slice(0, separatorIndex);
            }(topts.filename)}` + (ctx.opts.sourceMaps ? "+map" : "") + (topts.interopDefault ? ".i" : "") + `.${hash(topts.filename)}` + (topts.async ? ".mjs" : ".cjs");
            topts.jsx && topts.filename.endsWith("x") && (cacheName += "x");
            const cacheDir = ctx.opts.fsCache, cacheFilePath = pathe_M_eThtNZ_join(cacheDir, cacheName);
            if (!ctx.opts.rebuildFsCache && (0, external_node_fs_namespaceObject.existsSync)(cacheFilePath)) {
        ...
        function prepareCacheDir(ctx) {
            if (!0 === ctx.opts.fsCache && (ctx.opts.fsCache = function(ctx) {
                ...
                if (nmDir && (0, external_node_fs_namespaceObject.existsSync)(nmDir)) return pathe_M_eThtNZ_join(nmDir, ".cache/jiti");
                ...
                return pathe_M_eThtNZ_join(_tmpDir, "jiti");
        ...
        function eval_evalModule(ctx, source, evalOptions = {}) {
            const id = evalOptions.id || (evalOptions.filename ? basename(evalOptions.filename) : `_jitiEval.${evalOptions.ext || (evalOptions.async ? "mjs" : "js")}`), filename = evalOptions.filename || jitiResolve(ctx, id, {
                ...
                for(; path && "." !== path && "/" !== path;){
                    path = pathe_M_eThtNZ_join(path, "..");
                    try {
                        const pkg = (0, external_node_fs_namespaceObject.readFileSync)(pathe_M_eThtNZ_join(path, "package.json"), "utf8");
                        try {
            ...
            let compiled;
            mod.require = _jiti, mod.path = pathe_M_eThtNZ_dirname(filename), mod.paths = external_node_module_namespaceObject.Module._nodeModulePaths(mod.path), cache[filename] = mod, ctx.opts.moduleCache && (ctx.nativeRequire.cache[filename] = mod);
            ...
            let evalResult;
            try {
                evalResult = compiled(mod.exports, mod.require, mod, mod.filename, pathe_M_eThtNZ_dirname(mod.filename), _jiti.import, _jiti.esmResolve);
            } catch (error) {
        ...
        function createJiti(filename, userOptions = {}, parentContext, isNested = !1) {
            const opts = isNested ? userOptions : function(userOptions) {
                ...
                const deprecatedOverrides = {};
                return void 0 !== userOptions.cache && (deprecatedOverrides.fsCache = userOptions.cache), void 0 !== userOptions.requireCache && (deprecatedOverrides.moduleCache = userOptions.requireCache), {
                    ...jitiDefaults,
                    ...deprecatedOverrides,
                    ...userOptions
                };
            ...
            filename || (filename = process.cwd()), !isNested && isDir(filename) && (filename = pathe_M_eThtNZ_join(filename, "_index.js"));
            const url = pathToFileURL(filename), additionalExts = [
                ...opts.extensions
            ].filter((ext)=>".js" !== ext), nativeRequire = parentContext.createRequire(("TURBOPACK compile-time falsy", 0) ? "TURBOPACK unreachable" : filename), ctx = {
                filename,
                url,
                opts,
                alias,
...
//# sourceMappingURL=node_modules_jiti_dist_jiti_cjs_3caf0af2._.js.map